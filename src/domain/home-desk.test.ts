...
import {homeDeskCombinations,homeDeskFixture} from "../data/home-desk-fixtures";
import {assetsById} from "../data/catalog";
import {fitsSurface,footprint,footprintsOverlap,sizeOf} from "./placement";
import {homeDesk,homeDeskDestinations} from "../data/home-desk";
...
it("공부 책상 42조합의 전후 실제 지지면과 물건 간격",()=>{
  expect(homeDeskCombinations).toHaveLength(42);
  const failures=new Set<string>();
  for(const extras of homeDeskCombinations)for(const organized of [false,true]){
    const m=homeDeskFixture(extras,organized);
...
it("책의 전환은 입구 밖에서 끝나고 끌 때 잡은 원화를 유지한다",()=>{
  const before=homeDeskFixture([],false),after=homeDeskFixture([],true);
  for(const id of ["b1"]){
    const item=after.items.find(i=>i.id===id)!;
    const stored=after.placements[id],floor=before.placements[id];
    expect(dragPlacement(item,{...stored,x:400,y:600},homeDesk,"home-desk").surface).toBe(stored.surface);
    for(const reverse of [false,true])for(let i=0;i<=100;i++){
      const frame=motionFrame({id,from:reverse?stored:floor,to:reverse?floor:stored,progress:i/100},item,homeDesk,"home-desk");
      ...
      if(frame.inside&&frame.surface.bookSpines){
        ...
        expect(visualBounds(item,frame.placement,frame.surface,"home-desk").top-frame.elevation).toBeGreaterThanOrEqual(frame.surface.ceilingY!);
      }
    }
  }
});
it("공부 책상 42조합 모두 실제 이동 검증·복원·정리·청소·퀴즈 전이를 통과한다",()=>{
  const previous=physicalMaps["home-desk"]; physicalMaps["home-desk"]=homeDesk;
  try {
    for(const extras of homeDeskCombinations) {
      ...
      state=reducer(state,{type:"MAP",mapId:"home-desk"});
      ...
      for(const item of activeItems(state.mapId!,extras)) {
        const p=homeDeskDestinations[item.id];
  ...
  } finally {if(previous)physicalMaps["home-desk"]=previous;else delete physicalMaps["home-desk"];}
});

it("42조합에서 처음 각도를 유지한 채 입구를 눌러도 서랍 바닥까지 들어간다",()=>{
 const previous=physicalMaps["home-desk"];physicalMaps["home-desk"]=homeDesk;
 try { for(const extras of homeDeskCombinations){
  let state=reducer(makeSession(),{type:"BEGIN"});
  state=reducer(state,{type:"PROFILE",name:"검수",character:"rabbit"});
  state=reducer(state,{type:"TUTORIAL_DONE"});state=reducer(state,{type:"MAP",mapId:"home-desk"});
  for(const id of extras)state=reducer(state,{type:"EXTRA",id});
  state=reducer(state,{type:"CONFIRM_SET"});
  for(const item of activeItems("home-desk",extras)){
   const p=homeDeskDestinations[item.id],surface=homeDesk.surfaces.find(s=>s.id===p.surface)!;
