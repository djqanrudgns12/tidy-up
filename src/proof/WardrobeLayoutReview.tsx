...
export function WardrobeLayoutReview() {
  ...
  async function makeSheets() {
    ...
    try { for(const organized of [false,true]) for(let page=0;page<7;page++) {
      ...
      results.push({name:`wardrobe-${organized?"after":"before"}-${page+1}.png`,url:await canvasURL(sheet)});sheet.width=0;
    }
    ...
    for(const item of after.items)for(const reverse of [false,true]){
      ...
      results.push({name:`${"wardrobe"}-motion-${item.id}-${reverse?"out":"in"}.png`,url:await canvasURL(sheet)});sheet.width=0;
      await new Promise<void>(resolve=>window.setTimeout(resolve,0));
    }
  ...
  return <main className="app-shell placement-review">
    ...
    <h1>옷장 전체 배치 검수</h1>
    <p>옷장 활동의 42조합과 크기·가림·수납 경로를 확인합니다.</p>
    ...
    {thumbnail&&<a href={thumbnail} download="wardrobe-base-thumbnail-source.png">기본8개 썸네일 원본</a>}
    ...
    <figure><canvas ref={canvas} width={960} height={720} aria-label="옷장 전체 물건 배치 검수" /></figure>
  </main>;
}
