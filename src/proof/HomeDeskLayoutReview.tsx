...
export function HomeDeskLayoutReview() {
  ...
  const fixture=homeDeskFixture(homeDeskCombinations[choice],organized);
  useEffect(()=>{
    if(!art||!canvas.current)return;
    const before=homeDeskFixture(homeDeskCombinations[choice],false);
    const after=homeDeskFixture(homeDeskCombinations[choice],true);
    ...
    const base=renderLogicalScene({...homeDeskFixture([],false),art,showDirt:false});
  ...
  async function makeSheets() {
    ...
    try { for(const organized of [false,true]) for(let page=0;page<7;page++) {
      ...
      for(let cell=0;cell<6;cell++) {
        ...
        const model={...homeDeskFixture(homeDeskCombinations[index],organized),art,showDirt:false};
        ...
        ctx.fillStyle="#20352d";ctx.font="15px sans-serif";ctx.fillText(`${index+1}: ${homeDeskCombinations[index].join(",")||"기본8개"}`,x+8,y+18);
        ctx.drawImage(view,x,y+26,480,360);
        view.width=0;exported.width=0;
      }
      results.push({name:`homeDesk-${organized?"after":"before"}-${page+1}.png`,url:sheet.toDataURL("image/png")});sheet.width=0;
    }
  ...
  return <main className="app-shell placement-review">
    ...
    <h1>공부 책상 전체 배치 검수</h1>
    <p>공부 책상 활동의 42조합과 크기·가림·수납 경로를 확인합니다.</p>
    ...
    <label>추가 물건 조합 <select aria-label="추가 물건 조합" value={choice} onChange={e=>{setChoice(+e.target.value);setMoving("");}}>
      {homeDeskCombinations.map((ids,i)=><option key={i} value={i}>{i+1}: {ids.join(",")||"없음"}</option>)}
    </select></label>
    ...
    {thumbnail&&<a href={thumbnail} download="homeDesk-base-thumbnail-source.png">기본8개 썸네일 원본</a>}
    {sheets.map(sheet=><a key={sheet.name} href={sheet.url} download={sheet.name}>{sheet.name} </a>)}
    <figure><canvas ref={canvas} width={960} height={720} aria-label="공부 책상 전체 물건 배치 검수" /></figure>
  </main>;
}
